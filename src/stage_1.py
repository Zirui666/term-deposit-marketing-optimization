...
# =========================================================
# GLOBAL CONFIG & ORGANIZED PROJECT DIRECTORIES
# =========================================================
CONFIG = 10
...
CLEAN_DATA_DIR = PROJECT_ROOT / "10_Data_Clean"
EDA_OUTPUT_DIR = PROJECT_ROOT / "20_Data_Exploration"
MODEL_OUTPUT_DIR = PROJECT_ROOT / "30_Modeling_Outputs"
OPT_OUTPUT_DIR = PROJECT_ROOT / "40_Optimization_Results"

for folder in [RAW_DATA_DIR, CLEAN_DATA_DIR, EDA_OUTPUT_DIR, MODEL_OUTPUT_DIR, OPT_OUTPUT_DIR]:
    folder.mkdir(exist_ok=True)

# Default output directory used by helper functions.
# Specific outputs below are routed to more detailed folders.
...
def create_summary_table(data, name):
    ...
    file_path = str(EDA_OUTPUT_DIR / f"summary_stats_{name}.csv")
...
def main():
    ...
    # =========================================================
    old_path = RAW_DATA_DIR / "bank-full.csv"
    new_path = CLEAN_DATA_DIR / "bank_marketing_refined_with_synth_apr (cleaned records).csv"

    if not old_path.exists():
        raise FileNotFoundError(f"Missing required raw data file: {old_path}")
    if not new_path.exists():
        raise FileNotFoundError(f"Missing required cleaned data file: {new_path}")
    ...
    missing_by_col.to_csv(str(EDA_OUTPUT_DIR / "missing_by_column.csv"), index=False)
    print("Saved:", str(EDA_OUTPUT_DIR / "missing_by_column.csv"))
    ...
    missing_by_t.to_csv(str(EDA_OUTPUT_DIR / "missing_rows_by_num_calls.csv"), index=False)
    ...
    plt.tight_layout()
    plt.savefig(str(EDA_OUTPUT_DIR / "call_distribution.png"))
    plt.close()
    ...
    distribution_summary.to_csv(str(EDA_OUTPUT_DIR / "liquidity_risk_distribution.csv"))
    cluster_profile.to_csv(str(EDA_OUTPUT_DIR / "liquidity_risk_profiles.csv"))
    ...
    plt.tight_layout()
    plt.savefig(str(EDA_OUTPUT_DIR / "term_deposit_distribution_disaggregateddata.png"))
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(str(EDA_OUTPUT_DIR / "term_deposit_distribution_original.png"))
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(str(EDA_OUTPUT_DIR / "correlation_heatmap.png"))
    plt.close()

    disaggregated_df.to_csv(str(EDA_OUTPUT_DIR / "bank_disaggregated_long.csv"), index=False)
    ...
    t2_path = str(EDA_OUTPUT_DIR / "t2_slice_sorted.csv")
    ...
    plt.tight_layout()
    output_filename = str(EDA_OUTPUT_DIR / "sliced_correlation_heatmap.png")
    ...
    with open(str(MODEL_OUTPUT_DIR / "conversion_model_training_summary.txt"), "w") as f:
    ...
    print(evaluation_df)
    evaluation_df.to_csv(str(MODEL_OUTPUT_DIR / "conversion_model_evaluation_full.csv"), index=False)
    ...
    baseline_df["p_base_no_call"].describe().to_csv(str(MODEL_OUTPUT_DIR / "baseline_no_call_probability_summary.csv"))

    output_filename = str(MODEL_OUTPUT_DIR / "synthesized_deposit_summary.csv")
    ...
    dep_results_df.to_csv(str(MODEL_OUTPUT_DIR / "deposit_model_comparison.csv"), index=False)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(str(MODEL_OUTPUT_DIR / "deposit_roc_all_models.png"), dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(str(MODEL_OUTPUT_DIR / "conversion_roc_all_models.png"), dpi=300)
    plt.close()

    # =========================================================
    # SAVE BEST EBM CONVERSION MODEL ONLY
    # =========================================================
    ebm_conv_output_dir = str(MODEL_OUTPUT_DIR / "ebm_conversion_model")
    ...
    decision_grid.to_csv(str(OPT_OUTPUT_DIR / "decision_grid_fixed.csv"), index=False)
...
def run_stage_2():
    try:
        ...
        grid_path = str(OPT_OUTPUT_DIR / "decision_grid_fixed.csv")
        ...
        final_solution, EBIT_mat, APR_mat = stage_2.run_optimization_pipeline(decision_grid, output_dir=str(OPT_OUTPUT_DIR))
        final_solution.to_csv(OPT_OUTPUT_DIR / "FINAL_OPTIMAL_OFFERS.csv", index=False)
        print(f"\n✅ ALL DONE! Final offers saved to {OPT_OUTPUT_DIR / 'FINAL_OPTIMAL_OFFERS.csv'}")
